...
void fristDesign() {
    ...
    renderBitmapString(30 - 5, 50 - 40, (void *)font3, "COMPUTER GRAPHICS");
    renderBitmapString(30 - 8, 50 - 43, (void *)font3, "PROJECT");
}
...
void spe_key(int key, int x, int y) {
    switch (key) {
        ...
        case GLUT_KEY_LEFT:
            if (lrIndex >= 0) {
            // Increase movement speed by 2x (original: FPS/10)
                lrIndex = lrIndex - (FPS / 5);  // Changed from FPS/10
                if (lrIndex < 0) lrIndex = 0;    // Better boundary check
            }
            break;
        case GLUT_KEY_RIGHT:
            if (lrIndex <= 44) {
            // Increase movement speed by 2x (original: FPS/10)
                lrIndex = lrIndex + (FPS / 5);  // Changed from FPS/10
                if (lrIndex > 44) lrIndex = 44;  // Better boundary check
            }
            break;
    }
}
